...
description: >
  ...
  as scarce "voltage", hairline ink elevation (no heavy shadow), PingFang SC for all
  display + PingFang SC body + SF Mono for the index/code voice on a warm-navy code surface,
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "PingFang SC", cqw: 1.5, weight: 400, lineHeight: 1.5 }
  lead:    { fontFamily: "PingFang SC", cqw: 2.08, weight: 400, lineHeight: 1.5 }
  card-title:{ fontFamily: "PingFang SC", cqw: 2.3, weight: 500, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "PingFang SC", cqw: 1.46, weight: 500, lineHeight: 1.0 }
  tag-upper:{ fontFamily: "PingFang SC", cqw: 1.35, weight: 500, tracking: "0.18em", upper: true }
  kicker:  { fontFamily: "SF Mono", px: 28, cqw: 1.46, weight: 500, tracking: "0.16em", upper: true }
  mono-label:{ fontFamily: "SF Mono", px: 26, cqw: 1.35, weight: 500, tracking: "0.02em" }
  code:    { fontFamily: "SF Mono", cqw: 1.67, weight: 400, lineHeight: 1.6 }
  # — display ramp (PingFang SC 400, sentence case, negative tracking. Renderer embeds only 400/700 — author at 400; italic is the synthesized slant) —
  headline:{ fontFamily: "PingFang SC", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "PingFang SC", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "PingFang SC", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "PingFang SC", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "PingFang SC", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "PingFang SC", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.028em" }
  number-unit:{ fontFamily: "SF Mono", cqw: 2.08, weight: 500, lineHeight: 1.0 }
...
components:
  ...
  kicker-spike:
    ...
    description: "The eyebrow — SF Mono uppercase, indexical (2–5 words), prefixed with the coral ✱. Never plain text, never a sentence."
  coral-callout:
  ...
  number-lockup:
    ...
    description: "Hero stat — a PingFang SC figure paired with a SF Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (PingFang SC italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. PingFang SC italic, small PingFang SC uppercase cite beneath."
  section-rule:
  ...
  code-surface:
    ...
    textColor: "{colors.cream} (SF Mono); syntax in coral (keywords) / teal #5DB8A6 (strings) / amber #E8A55A (numbers)"
...
- **Fonts** — already set to **PingFang SC** (display) / **-apple-system** (body); ignore any preset font name lingering in prose.
...
Three editorial voices, each in its own face: **PingFang SC** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **PingFang SC** carries body, leads, card titles, buttons, and
UI chrome. **SF Mono** carries the indexical layer — kickers, technical labels, the code
...
- **PingFang SC** (sentence case, negative-tracked) for all display; **PingFang SC** body/chrome; **SF Mono** index + code.
...
- **Squint** — one PingFang SC display moment dominates at 3–6× its neighbor.
...
- **Type** — PingFang SC sentence-case display (negative-tracked); PingFang SC 400 body; SF Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (PingFang SC `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (PingFang SC `headline` 4.6cqw
...
- **PingFang SC display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **PingFang SC body** sentence case weight 400. **SF Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — PingFang SC figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — PingFang SC italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized PingFang SC · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line PingFang SC `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** PingFang SC 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single PingFang SC line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line PingFang SC `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, number-lockup, lead/caption, optional `section-rule`. **Focal** a PingFang SC `number-hero` figure with a mono unit over a 1px rule — the PR impact (`+1,204 / −318`, `17 files`, `2.1× faster`). **Chrome** mono tag. **Accent** the figure in ink; at most one coral unit. **Fixed** serif figure + mono unit, hairline rule. **Free** the figures (from the script), tag, layout + density. **Density** free.

### 5 · Pull-quote (quote · move: PingFang SC italic · cream)

**Ground** `{colors.cream}`. **Composes** kicker-spike, pull-quote, tag-upper cite. **Focal** a PingFang SC **italic** `quote-pull` — a commit message, a reviewer line, or the thesis — with a small PingFang SC uppercase cite (author · role) beneath. **Chrome** mono kicker. **Accent** none, or one coral mark. **Fixed** PingFang SC italic quote + uppercase cite. **Free** quote, attribution, layout + density. **Density** free.

### 6 · Closing / CTA (closer · move: coral voltage · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}`). **Composes** display sign-off, coral-callout, optional contributor row (`assets/<login>.png` avatars + mono names). **Focal** a short PingFang SC sign-off with the one **coral-callout** (the CTA or full-bleed band) and, for a "shipped-by" close, a row of hairline-ringed avatar chips. **Chrome** mono index. **Accent** the single coral voltage. **Fixed** one coral moment, hairline avatar rings, sentence-case serif. **Free** sign-off, who ships, layout + density. **Density** free.
...
- Set all display in **PingFang SC, sentence case**, negative-tracked; **PingFang SC 400** body; **SF Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a PingFang SC figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case PingFang SC display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one PingFang SC display moment dominates.
...
- **Type** — PingFang SC sentence-case display (negative-tracked); PingFang SC 400 body; SF Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
